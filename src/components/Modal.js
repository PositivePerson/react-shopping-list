...
import { MDBInput, MDBBtn, MDBModal, MDBModalBody, MDBContainer, MDBModalHeader, MDBModalFooter } from 'mdbreact';
...
const Modal = ({
		...
		() => {
			if (editing) {
				if (productContext.currentItem.pieces || productContext.currentItem.weight) {
					let value =
						productContext.currentItem.pieces > 0
							? productContext.currentItem.pieces
							: productContext.currentItem.weight;
	...
	const addItem = (e) => {
		...
		pieces
			? globalPiecesCounter(globalPieces + parseInt(pieces))
			: globalWeightCounter(globalWeight + parseInt(weight));
				...
				<MDBModalBody className=' mb-1'>
					<div className='md-form form-sm w-50'>
						<MDBInput
							...
							id='form2'
							ref={productNameRef}
							valueDefault={editing ? productContext.currentItem.name : undefined}
							required
						/>
					</div>
